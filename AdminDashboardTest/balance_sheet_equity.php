...
$result_equity = false; // Initialize with a default value
...
if (isset($_GET['start_date']) && isset($_GET['end_date'])) {
    ...
    if ($start_date <= $end_date) {
        $query_equity = "SELECT * FROM equity WHERE invoice_date BETWEEN '$start_date' AND '$end_date' ORDER BY type, invoice_date";
        $result_equity = $conn->query($query_equity);
    }
}
?>
...
<div class="recent-orders"
    ...
    <h2 style="text-align: center; margin-bottom: 20px; cursor: pointer;" onclick="toggleCollapse('equity-matrix')">
        Equity</h2>
    <div id="equity-matrix" style="display: none;">
        <table style="width: 100%; border-collapse: collapse;">
            ...
            <tbody>
                ...
                $type_total_equity = 0;
                $total_equity = 0; // Initialize cumulative total
                $index_receivable = 0; // Index for unique collapsible IDs
                
                // Check if the query returned valid results
                if ($result_equity && $result_equity->num_rows > 0) {
                    while ($row_receivable = $result_equity->fetch_assoc()) {
                        if ($current_type_receivable != $row_receivable['type']) {
                            if ($current_type_receivable !== null) {
                                // Display the total for the previous type
                                echo "<tr><td colspan='4' style='padding: 8px; text-align: right; font-weight: bold; background-color: #e9e9e9;'>Total: " . number_format($type_total_equity, 2) . "</td></tr>";
                                echo "</div>"; // Close the collapsible content
                
                                // Add type total to cumulative total
                                $total_equity += $type_total_equity;
                            }
                        ...
                        $type_total_equity += $row_receivable['amount_paid'];
                    ...
                    // Display the total for the last type and add it to the cumulative total
                    echo "<tr><td colspan='4' style='padding: 8px; text-align: right; font-weight: bold; background-color: #e9e9e9;'>Total: " . number_format($type_total_equity, 2) . "</td></tr>";
                    echo "</div>";
                    $total_equity += $type_total_equity;
                    echo "<tr><td colspan='4' style='padding: 8px; text-align: right; font-weight: bold; background-color: #e9e9e9;'>Total Equity: " . number_format($total_equity, 2) . "</td></tr>";
                    // Close the last collapsible content
        ...
        <table>
            <?php
            $total_combined = (isset($total_liabilities) ? $total_liabilities : 0) + (isset($total_equity) ? $total_equity : 0);

            echo "<tr><td colspan='4' style='padding: 8px; text-align: center; font-weight: bold; background-color: #d9d9d9;'>Total Liabilities and Equity: " . number_format($total_combined, 2) . "</td></tr>";
            ?>
        </table>
    </div>
</div>